...
build_gnutls() {
  download_and_unpack_file http://www.mirrorservice.org/sites/ftp.gnupg.org/gcrypt/gnutls/v3.5/gnutls-3.5.8.tar.xz
  cd gnutls-3.5.8
...
build_freetype() {
  download_and_unpack_file https://download.savannah.gnu.org/releases/freetype/freetype-2.7.1.tar.gz
  cd freetype-2.7.1
...
build_sdl2() {
  ...
  cd SDL2-2.0.5
    generic_configure
    # apply_patch $SCRIPT_DIR/patches/sdl2.xinput.diff only needed if we are using mingw-w64 master
    do_make_and_make_install
  cd ..
  reset_cflags
  mkdir -p temp
